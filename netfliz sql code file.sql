select * from netflix_raw group by title

--handling the foreign characters  (nvarchar)
--remove duplicates
...
--new table for listed_in,cast,director,country
...
--data typed conversion for data added 
with cte as (
select * 
,ROW_NUMBER() over(partition by title , type order by show_id) as rn
from netflix_raw 
...
select * from netflix
--populate missing va;ues in country and duration columns
...
--populate rest of the null as not_available
with cte as (
select * 
,ROW_NUMBER() over(partition by title , type order by show_id) as rn
from netflix_raw
...
Select 
  ...
  COUNT(distinct case when n.type = 'TV Shows' then n.show_id end ) as no_of_Tvshows
From netflix n
